...
/**
 * Create a Helicopter and the Helipads
...
// The global variables for the helipad
let helipadCount = 0;
let helipadMaterial;
let helipadGeometry;
...
export class Helipad extends GrObject {
  ...
  constructor(x, y, z) {
    if (!helipadGeometry) {
      ...
      const padcoords = [-1, 0, -1, -1, 0, 1, -h, 0, 1, -h, 0, -1, 1, 0, -1, 1, 0, 1, h, 0, 1, h, 0, -1, -h,
        0, -q, -h, 0, q, h, 0, q, h, 0, -q,
      ];
...
export class Helicopter extends GrObject {
  ...
  constructor(params = {}) {
    ...
    const helifaces = [0, 1, 4, 1, 2, 4, 2, 3, 4, 3, 0, 4, 1, 0, 5, 2, 1, 5, 3, 2,
      5, 0, 3, 5, 6, 7, 10, 7, 8, 10, 8, 9, 10, 9, 6, 10,
    ];
    ...
    this.body.scale.set(1, 0.7, 1.3);
    this.body.castShadow = true;
    this.helicopter.add(this.body);

    this.rotorGeom = new T.Geometry();
    const rotorcoords = [0, body, 0, rotor, body, 0.1, rotor, body, -0.1, -rotor, body, 0.1, -rotor, body, -0.1,];
    ...
    let mat = new T.MeshStandardMaterial({ color: "skyblue", metalness: 0.5 });
    this.rotor = new T.Mesh(this.rotorGeom, mat);
    this.rotor.position.y = 0;
    this.rotor.scale.set(1, 1.5, 1);
    this.helicopter.add(this.rotor);

    this.copy = this.rotor.clone();
    this.copy.rotateY(3);
    this.helicopter.add(this.copy);
    ...
    //
    this.altitude = params.altitude || 10;
   ...
   * it doesn't know about the world, pass it a list of objects
  ...
  tick(delta, timeOfDay) {
    ...
    this.copy.rotateY(deltaSlowed * 4);
        ...
        case 1: // ascend to altitude
          ...
          if (this.helicopter.position.y >= this.altitude) {
            ...
            if (ds > 0) {
              ...
              this.state = 2; // don't bother spinning
            }
          }
          break;
        case 2: // descend
